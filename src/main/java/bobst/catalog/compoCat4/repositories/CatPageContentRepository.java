...
public interface CatPageContentRepository extends CrudRepository<CatPageContent, Long> {
	...
	@Query("select new bobst.catalog.compoCat4.models.Part(c.repere, c.idItem, c.contentType, i.descriptionFr, i.descriptionEn) from CatPageContent c, CatItem i where c.idItem = i.idItem and c.idPage = ?1 order by length(c.repere),c.repere")
